...
* 链码设计（苹果树，包裹的查询，修改）
* 接口设计（创建用户、发送包裹、获取苹果树信息、查询包裹信息、添加苹果树、添加包裹等）
* 学习React框架，学习dva应用流框架，调用后端接口。

下周目标：

* 继续学习React框架，学习如何写组件，以及对数据的渲染

## 3.20-3.26

* 实现前端获取后端数据，并完成初步的数据渲染
  * 学习了dva框架，主要用于获取远程接口数据，以及如何在页面上通过`this.props`的属性渲染数据
* 重新设计了接口，规范化方法名以及注释等，同时完成了API的接口文档
  * 通过EggJs框架实现了控制器、服务分离，便于阅读与调试

* 重新设计了链码（智能合约），将方法设计抽象化，增强可移植性
* 前端实现产品线以及包裹查询功能，初步UI渲染成功
...
* 完成用户登录模块
* 完成权限管理模块：动态路由、动态菜单（即特定权限的使用者只能访问页面，进行鉴权）
* 完成苹果树、苹果、包裹新建功能
* 完成用户管理页面（修改、删除用户信息）

下周目标：

* 升级区块链数据库为CouchDB（支持富查询）：暂时不需要
* * 管理员页面显示产品的批次信息（通过MySQL数据库实现）
* 增加二维码读取与生成功能

## 4.3-4.9

* 新建`firstLevel`和`secondLevel`mysql表，用于初步显示第一层级和第二层级数据，记录链上数据的基本信息，便于页面显示
* 实现二维码扫描功能
  * 提交二维码前校验二维码内容
* 将前端项目部署到eggJS项目中
  * 学习了静态资源注册的相关知识
  * 了解了哈希路由和浏览器路由两种方式及区别
* 完成网页端交互区块链功能
  * 添加数据上链
  * 更新数据上链

* 信息页展示数据的同步（第二层级数据展示时其所属第一层级的信息展示，原先为默认字符串展示）
* 前端页面布局优化
...
* 合并`产品线查询`和`阶段查询`，根据编号前缀进行分类显示
  * 将输入或者通过二维码获得的ID进行区块链查询，有才显示，无就报错
* 完成二维码下载按钮
* 更新检索的校对规则（如数据未上链进行提示、检索编号若不正确则进行提示）

* 修改后端操作Fabric的源码，将上链的用户和组织进行修改（原先是设置为默认的`admin和org1`）
* 完善剩余组织(`org2, org3, org4`)的网关配置，即连接配置文件。（在`prodTrace-Backend/app/public/gateway`中）
* 修复页面刷新bug：层级数据具体操作页面刷新无法获取值

* 设计轮播式的溯源信息展示

* 优化溯源信息展示页面，更新网站名称和logo
* 优化表格组件，抽取为一个通用组件

## 4.17-4.23

* 修复权限的安全问题：
  * 之前是以明文显示，修复后通过AES加密方式进行暴露，通过自定义密钥进行解密获取原本权限值。

![image-20210423144955433](MarkdownAssets/README/image-20210423144955433.png)

![image-20210423145012578](MarkdownAssets/README/image-20210423145012578.png)
...
* 添加上链状态字段，如果未上链则可以进行删除，上链后则无法删除

  ![image-20210509074452955](MarkdownAssets/README/image-20210509074452955.png)
