...
>
> 使用Sequelize创建表的流程：[链接](https://www.jianshu.com/p/b0eb45a8ae5e?from=groupmessage)，中文文档[链接](https://itbilu.com/nodejs/npm/sequelize-docs-v5.html)
>
> 
...
| ---------- | ---------------------------- | -------- | ---- |
| id         | 用户账号                     | string   | 主键 |
| org        | 区块链所在组织(org1~org4)    | string   |      |
| role       | 身份权限(guest, user, admin) | string   |      |
| name       | 姓名                         | string   |      |
| password   | 密码                         | string   |      |
...
| ---------- | -------- | -------- | ---------------------------------------- |
| id         | 批次编号 | string   | 主键                                     |
| type       | 批次类型 | string   | 批次类型，是产品源（ori）还是包裹（pkg） |
| name       | 批次名称 | string   |                                          |
| num        | 批次数量 | integer  | 即产生多少该批次的记录                   |
| created_at | 创建时间 | date     |                                          |
| created_at | 更新时间 | date     |                                          |
| TODO       | 模板编号 | string   |                                          |
| TODO       | 模版名称 | string   |                                          |
...
| name            | 二级层级名称               | string   |                                |
| num             | 批次数量                   | integer  | 即产生多少该批次的记录         |
| firstLevelOriId | 所属一级层级编号（产品源） | string   | 必须从第一层级里的产品源里取值 |
| firstLevelPkgId | 所属一级层级编号（包裹）   | string   | 可为空                         |
| created_at      | 创建时间                   | date     |                                |
| created_at      | 更新时间                   | date     |                                |
| TODO            | 模板编号                   | string   |                                |
| TODO            | 模版名称                   | string   |                                |
